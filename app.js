...
function addPackToCart() {
  const packProducts = [1, 2, 3]; // camiseta, tirantes, dorsal
  packProducts.forEach(id => {
    const product = products.find(p => p.id === id);
    if (!product) return;
    const existing = cart.find(item => item.id === id && item.size === 'M');
    if (existing) { existing.qty++; }
    else {
      cart.push({ id, name: product.name, price: product.price, image: product.image, size: 'M', qty: 1 });
    }
  });
  saveCart();
  updateCartUI();
